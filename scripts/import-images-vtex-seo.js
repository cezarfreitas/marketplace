// Script para importar imagens da VTEX para a tabela images_vtex no banco seo_data
...
async function importImagesVtex() {
  console.log('🚀 Iniciando importação de imagens da VTEX para banco seo_data...');
  ...
  try {
    ...
    const [tables] = await connection.execute(`
      ...
      WHERE TABLE_SCHEMA = 'seo_data' AND TABLE_NAME = 'images_vtex'
    `);
    
    if (tables.length === 0) {
      console.log('❌ Tabela images_vtex não existe. Execute primeiro o script create-images-vtex-table-seo.js');
      return;
    }
...
async function insertImageVtex(connection, image) {
  ...
  if (values[0] === null || values[1] === null || values[2] === null || values[3] === null || values[8] === null) {
    throw new Error('Valores obrigatórios não podem ser null');
  }
...
async function importImagesForSku(skuId) {
  console.log(`🚀 Importando imagens para SKU ${skuId} no banco seo_data...`);
