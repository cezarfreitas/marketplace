...
async function updateAgent6NoPalavraApresentacao() {
  let connection;
  ...
  try {
    ...
    // Novo system prompt mantendo a seção mas sem a palavra "APRESENTAÇÃO"
...
1. SEÇÃO INICIAL
   ...
   - Apresentar o produto de forma envolvente usando o TÍTULO OTIMIZADO
...
[Parágrafo introdutório atrativo baseado na análise da imagem e título otimizado]<br><br>
...
1. Seção inicial (parágrafo introdutório baseado na análise visual e título otimizado)
...
- NÃO use emojis ou ícones em lugar nenhum
- NÃO use formatação visual além das tags básicas
- Começar com parágrafo introdutório SEM título de seção`;

    console.log('Atualizando agente ID 6 para remover apenas a palavra APRESENTAÇÃO...');
    
    const [updateResult] = await connection.execute(`
      UPDATE agents 
      SET 
        name = 'Especialista em Descrições HTML Sem Palavra Apresentação',
        system_prompt = ?,
        guidelines_template = ?,
        max_tokens = 2000,
        temperature = 0.7,
    ...
    console.log('Estrutura HTML final (sem palavra APRESENTAÇÃO):');
    console.log('   [Parágrafo introdutório SEM título]<br><br>');
    ...
    console.log('   - Começar com parágrafo introdutório SEM título');
...
// Executar
updateAgent6NoPalavraApresentacao();
