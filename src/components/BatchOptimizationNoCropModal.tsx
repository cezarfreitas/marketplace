...
interface BatchOptimizationNoCropModalProps {
  isOpen: boolean;
  onClose: () => void;
  selectedProducts: number[];
  onComplete: (results: BatchOptimizationResult[]) => void;
}

export function BatchOptimizationNoCropModal({ 
  isOpen, 
  onClose, 
  selectedProducts, 
  onComplete 
}: BatchOptimizationNoCropModalProps) {
  
  const [isProcessing, setIsProcessing] = useState(false);
  const [progress, setProgress] = useState(0);
  const [results, setResults] = useState<BatchOptimizationResult[]>([]);
  const [error, setError] = useState<string | null>(null);
  const [isCompleted, setIsCompleted] = useState(false);
  const [currentProduct, setCurrentProduct] = useState('');
  const [currentStep, setCurrentStep] = useState('');
  const [currentStepType, setCurrentStepType] = useState<'imageAnalysis' | 'titleGeneration' | 'descriptionGeneration' | 'characteristicsGeneration' | 'anymarketSync' | null>(null);
  const [expandedProduct, setExpandedProduct] = useState<number | null>(null);
  const [currentProductIndex, setCurrentProductIndex] = useState(0);

  const successCount = results.filter(r => r.success && r.steps?.imageAnalysis?.success && r.steps?.titleGeneration?.success && r.steps?.descriptionGeneration?.success && r.steps?.characteristicsGeneration?.success && r.steps?.anymarketSync?.success).length;
  const errorCount = results.filter(r => r.error || (r.steps?.imageAnalysis?.error) || (r.steps?.titleGeneration?.error) || (r.steps?.descriptionGeneration?.error) || (r.steps?.characteristicsGeneration?.error) || (r.steps?.anymarketSync?.error)).length;
  ...
  const handleStartProcessing = async () => {
    ...
    setCurrentProduct('');
    setCurrentStep('Preparando otimização...');

    try {
      // Usar a API de otimização em lote sem crop
      setCurrentStep('Iniciando otimização em lote (sem crop)...');
      
      const response = await fetch('/api/optimize-batch-no-crop-stream', {
        method: 'POST',
      ...
      setCurrentStep('Erro na otimização');
      setCurrentProduct('');
    } finally {
  ...
  const getStatusIcon = (result: BatchOptimizationResult) => {
    if (result.success) {
      return <CheckCircle className="w-4 h-4 text-green-500" />;
    } else if (result.error || (result.steps?.imageAnalysis?.error) || (result.steps?.titleGeneration?.error) || (result.steps?.descriptionGeneration?.error) || (result.steps?.characteristicsGeneration?.error) || (result.steps?.anymarketSync?.error)) {
  ...
  const ProgressIcons = ({ result }: { result: BatchOptimizationResult }) => {
    ...
    const currentStep = result.steps?.imageAnalysis?.message === 'Executando...' ? 'imageAnalysis' :
                       ...
                       result.steps?.anymarketSync?.message === 'Executando...' ? 'anymarketSync' : null;
  ...
  const getStatusBadge = (result: BatchOptimizationResult) => {
    if (result.success) {
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => {
      ...
      <DialogContent 
        ...
        aria-describedby="batch-optimization-description"
      ...
      >
        <DialogHeader>
          <DialogTitle>Otimização em Lote (Sem Crop)</DialogTitle>
        </DialogHeader>
        ...
        <div className="p-6">
          <div className="flex items-center justify-between mb-6">
            <div>
              <p 
                id="batch-optimization-description"
                className="text-gray-600 mt-1"
              >
                Análise de imagens, geração de títulos, descrições, características e sincronização AnyMarket para {selectedProducts.length} produto(s) selecionado(s) (sem crop de imagens)
              </p>
            </div>
            <Button
              variant="outline"
            ...
            <div className="mb-6">
              <div className="flex items-center justify-between mb-2">
                ...
                <span className="text-sm text-gray-500">
                  {results.length > 0 ? `${results.filter(r => r.success || (r.steps?.imageAnalysis?.error) || (r.steps?.titleGeneration?.error) || (r.steps?.descriptionGeneration?.error) || (r.steps?.characteristicsGeneration?.error) || (r.steps?.anymarketSync?.error)).length}/${totalSteps}` : '0'}/{totalSteps}
                </span>
              </div>
              <Progress 
                value={results.length > 0 ? (results.filter(r => r.success || (r.steps?.imageAnalysis?.error) || (r.steps?.titleGeneration?.error) || (r.steps?.descriptionGeneration?.error) || (r.steps?.characteristicsGeneration?.error) || (r.steps?.anymarketSync?.error)).length / totalSteps) * 100 : 0} 
                className="w-full" 
              />
            ...
            <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
              <div className="flex items-center">
                ...
                <div className="flex-1">
                  <h3 className="text-sm font-medium text-red-800">Erro na Otimização</h3>
          ...
          {results.length > 0 && (
            <div className="space-y-4">
              ...
              {results.map((result, index) => {
                ...
                const hasError = result.error || (result.steps?.imageAnalysis?.error) || (result.steps?.titleGeneration?.error) || (result.steps?.descriptionGeneration?.error) || (result.steps?.characteristicsGeneration?.error) || (result.steps?.anymarketSync?.error);
